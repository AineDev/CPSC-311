  ...
  (type-case s-A4L exp
    ...
    [s-withrecs (defns body)
                (withrecs (map (λ (defn)
                               ...
                               defns)
                          (desugar body))]
    [s-fun (ids base) (foldr (lambda (x acc) (fun x acc))
                             (desugar base)
                             ids)]
    ...
    ;      For functions in the line before. Oct 16th has desugaring
    ; Take the function and apply the desugared version with the desugared arguments
    ; look at Oct 16th desugaring & define-type compared to this
    [s-app (fn args) (app (desugar fn)
  ...
  (local [
          ...
          (define (cyclically-bind-and-interp name bound-expr env)
            (local ([define box-for-name (box #f)]
                    [define new-env (aRecEnv name box-for-name env)] ;AINE why?
          ...
          (define (helper expr env)
            (type-case A4L expr
              ...
              [if0 (scr thn els)
                   (if (= 0 (numV-n (is-a? numV? (helper scr env))))
                       (helper thn env)
                       (helper els env))]
              [mt () [mtV]]
