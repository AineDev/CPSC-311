...
(define (fermat-primality-test a n) ; 2 4
  (cond [(< a 2) false]
        [(> a n) false]
        [else (= (remainder (expt a (- n 1)) n) 1)]))
...
(define (match-length string1 string2)
  (local [(define (compare-strings string1 string2 min max)
            (cond [(<= (string-length string1) min) 0]
                  [(<= (string-length string2) min) 0]
                  [else 
                   (if (string=? (substring string1 min max) (substring string2 min max))
                       (+ 1 (compare-strings string1 string2 (add1 min) (add1 max)))
                       0)]))]
    (compare-strings string1 string2 0 1)))
...
(define (interleave list1 list2)
  (local [(define (interleave list1 list2 acc)
            (cond [(empty? list1) list2]
                  [(empty? list2) list1]
                  [else (if acc
                            (cons (first list1) (interleave (rest list1) list2 false))
                            (cons (first list2) (interleave list1 (rest list2) true)))]))]
    (interleave list1 list2 true)))
...
(define (contains-sequence data sequence)
  (cond [(empty? sequence) true]
        [(empty? data) false]
        [else
         (if (symbol=? (first data) (first sequence))
...
(test (all-thingied (other-thingy false true (other-thingy true true (a-thingy true)))) false)
(test (all-thingied (other-thingy true false (other-thingy true true (a-thingy true)))) false)
(test (all-thingied (other-thingy true true (other-thingy false true (a-thingy true)))) false)
(test (all-thingied (other-thingy true true (other-thingy true false (a-thingy true)))) false)
...
(define (find-species name tree)
  (type-case tree-of-life tree
    ...
    [species (n e c1 c2)
             (if (string=? name n)
                 tree
                 (if (false? (find-species name c1))
                     (find-species name c2)
                     (find-species name c1)))]))
; Oh my god this is so inefficient it hurts, I'm gonna try and make it better later (probably maybe)
...
(define (common-ancestor name1 name2 tree)
  (local [(define (both-names-in-child tree)
            (and (not (false? (find-species name1 tree)))
                 (not (false? (find-species name2 tree)))))
          
          (define (names-in-seperate-trees tree1 tree2)
            (or (and (and (not (false? (find-species name1 tree1)))
                      (false? (find-species name2 tree1)))
                 (and (not (false? (find-species name2 tree2)))
                      (false? (find-species name1 tree2))))
                (and (and (not (false? (find-species name2 tree1)))
                      (false? (find-species name1 tree1)))
                 (and (not (false? (find-species name1 tree2)))
                      (false? (find-species name2 tree2))))))
          
          (define (name-in-either-tree name tree1 tree2)
            (or (not (false? (find-species name tree1)))
                (not (false? (find-species name tree2)))))]
    
    (type-case tree-of-life tree
      [empty-tree () false]
      [species (n e c1 c2) (if (names-in-seperate-trees c1 c2) ;TODO: turn these into conds for readability
                               tree
                               (if (both-names-in-child c1)
                                   (common-ancestor name1 name2 c1)
                                   (if (both-names-in-child c2)
                                       (common-ancestor name1 name2 c2)
                                       (if (string=? n name1)
                                           (if (name-in-either-tree name2 c1 c2)
                                               tree
                                               (if (string=? n name2)
                                                   tree
                                                   false))
                                           (if (string=? n name2)
                                               (if (name-in-either-tree name1 c1 c2)
                                                   tree
                                                   false)
                                               false)))))])))
...
;; the language.
;;
;; <expr> ::= (group <expr> <expr>) ; 1
;;          | (sequentially <expr> <expr>) ; 2
;;          | (together <expr> <expr>) ; 3
;;          | (join <expr> <expr>) ; 4
;;          | (arrive <expr>) ; 5
;;          | (give <expr> name <id> in <expr>) ; 6
;;          | <string> ; 7
;;          | <id> ; 8
;;
...
;;    (sequentially <expr> <expr>). give 3 name x in x+1

(define-type OE
  [group (expr1 OE?) (expr2 OE?)] ; 1
  [sequentially (expr1 OE?) (expr2 OE?)] ; 2
  [together (expr1 OE?) (expr2 OE?)] ; 3
  [join (expr1 OE?) (expr2 OE?)] ; 4
  [arrive (expr1 OE?)] ; 5
  [give  (expr1 OE?) (expr2 id-ref?) (expr3 OE?) ] ; 6
  [string-literal (string-literal string?)] ; 7
  [id-ref (id-ref symbol?)]) ; 8
...
; create a variable that contains a comma-separated list of the non-terminals used to match that regexp (in "depth-first" order
; Subproblem 1: "(ab)|c"
